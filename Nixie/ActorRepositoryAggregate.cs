...
public sealed class ActorRepositoryAggregate<TActor, TRequest> : IActorRepositoryRunnable where TActor : IActorAggregate<TRequest> where TRequest : class
{
    ...
    private readonly ConcurrentDictionary<string, Lazy<(ActorRunnerAggregate<TActor, TRequest>, ActorAggregateRef<TActor, TRequest>)>> actors = new();
    ...
    {        
        foreach (KeyValuePair<string, Lazy<(ActorRunnerAggregate<TActor, TRequest> runner, ActorAggregateRef<TActor, TRequest> actorRef)>> actor in actors)
        {
            Lazy<(ActorRunnerAggregate<TActor, TRequest> runner, ActorAggregateRef<TActor, TRequest> actorRef)> lazyValue = actor.Value;
    ...
    public IActorAggregateRef<TActor, TRequest> Spawn(string? name = null, params object[]? args)
    {
        ...
        Lazy<(ActorRunnerAggregate<TActor, TRequest> runner, ActorAggregateRef<TActor, TRequest> actorRef)> actor = actors.GetOrAdd(
            name,
    ...
    private (ActorRunnerAggregate<TActor, TRequest>, ActorAggregateRef<TActor, TRequest>) CreateInternal(string name, params object[]? args)
    {
        ActorRunnerAggregate<TActor, TRequest> runner = new(actorSystem, logger, name);

        ActorAggregateRef<TActor, TRequest>? actorRef = (ActorAggregateRef<TActor, TRequest>?)Activator.CreateInstance(typeof(ActorAggregateRef<TActor, TRequest>), runner);
    ...
    /// <param name="name"></param>
    /// <returns></returns>
    public IActorAggregateRef<TActor, TRequest>? Get(string name)
    {
        name = name.ToLowerInvariant();

        if (actors.TryGetValue(name, out Lazy<(ActorRunnerAggregate<TActor, TRequest> runner, ActorAggregateRef<TActor, TRequest> actorRef)>? actor))
            return actor.Value.actorRef;
